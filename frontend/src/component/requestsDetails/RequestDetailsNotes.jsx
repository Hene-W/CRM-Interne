...
const RequestDetailsNotes = ({ request, updateRequest }) => {
    ...
    const hasChanges = internalNotes !== initialNotes;
    ...
    return (
        <div className="md:w-2/3 bg-white border rounded-xl p-4 flex flex-col flex-1 h-full overflow-hidden">
            ...
            <div className="flex justify-between items-center py-3 border-b bg-white">
                ...
                <button disabled={!hasChanges || isSaving} onClick={handleSaveNotes} className={`flex items-center justify-center gap-2 ${!hasChanges || isSaving ? "bg-gray-300 text-gray-500" : "bg-[#1f1f1f] hover:bg-[#333333] text-white"} rounded-lg px-3 py-2 w-auto`}>Sauvegarder</button>
            </div>
